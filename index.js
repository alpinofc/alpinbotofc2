...
case 'listonline':
			    ...
			    Pin.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('alpin', 'pinbot')} ${ran} -o ${ran}`, async (error) => {
...
case 'block':
  ...
  Pin.sendMessage(from, `Memblokir nomor, Perintah Diterima`, text, {
...
case 'unblock':
  ...
  Pin.sendMessage(from, `Membuka blokir, Perintah diterima`, text)
...
//--Hilih maker
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						exec(`webpmux -set exif ${addMetadata('alpin', 'pinbot')} ${rano} -o ${rano}`, async (error) => {
...
Pin.sendMessage(from, 'Cantikcek *'+cantik+'*\n\nJawaban : '+ tik+'%', text, {
